// pit 낙사 원인 진단 — 신발×3 로드아웃에서 pit 낙사 발생 시드 찾고
// 사망 직전 봇 상태 시퀀스를 로그로 남긴다. 일회성 진단 도구.
...
describe("pit 낙사 진단", () => {
  it("신발×3 첫 pit 낙사 케이스 사망 전 로그", () => {
    // 벤치와 동일한 rng 시퀀스로 pit 낙사 첫 발생 시드 찾기
    ...
    if (foundCycleSeed < 0) {
      console.log("pit 낙사 케이스 없음 (또는 MAX_MEASUREMENTS 부족)");
      return;
    }

    console.log(
      `측정 #${foundMeasurement}, 사이클 #${foundCycle}, cycleSeed=${foundCycleSeed}에서 첫 pit 낙사`,
    );

    // 재시뮬 — 사망 전 LOG_BUFFER_SIZE tick 로그 남기기
    const w = createWorld(foundCycleSeed, trackPool, loadout);
    console.log(
      `초기 트랙 id=${w.currentTrack.id} pits=[${(w.currentTrack.pits ?? []).map((p) => `${p.x.toFixed(0)}~${(p.x + p.width).toFixed(0)}`).join(",")}]`,
    );
    const buffer: string[] = [];
    let tick = 0;
    ...
    while (w.runner.alive) {
      ...
      if (w.currentTrack.id !== lastTrackId) {
        console.log(
          `트랙 전환 → id=${w.currentTrack.id} pits=[${(w.currentTrack.pits ?? []).map((p) => `${p.x.toFixed(0)}~${(p.x + p.width).toFixed(0)}`).join(",")}]`,
        );
        lastTrackId = w.currentTrack.id;
      }
      const pits = w.currentTrack.pits ?? [];
      // 봇 근처 pit 정보 (봇 x 기준 좌 200 ~ 우 500 안)
        ...
        .filter(
          (_, i) =>
            ...
            w.currentTrack.obstacles[i]!.x + w.currentTrack.obstacles[i]!.width >
              r.x - 100 &&
            w.currentTrack.obstacles[i]!.x < r.x + 300,
        )
        .map((o) => {
      ...
      // 봇 앞 아이템(정적 + spawned) — 봇 gap 범위 -50 ~ +300
    ...
    console.log(
      `사망 원인: pitFall=${w.lastPitFall}, collision=${w.lastCollision ? JSON.stringify(w.lastCollision) : "no"}, hp=${w.runner.hp.toFixed(1)}`,
    );
    console.log(
